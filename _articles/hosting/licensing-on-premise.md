...
1. Log in to your self-hosted Web Vault with an email address that matches the Cloud-hosted account from which you downloaded the license.
   ...
   {% callout success %}If you haven't already, verify your email address before proceeding. You will need to have [configured SMTP-related environment variables](/environment-variables) to do so.{% endcallout %}
...
{% callout info %}
You must be an [Organization Owner](/user-types-access-control) to both retrieve and apply a license.
{% endcallout %}
...
If you receive a `version not supported` error message, you'll need to update your server before proceeding. Make a backup or copy of the `bwdata` directory, then follow [these instructions](/updating-on-premise/).
