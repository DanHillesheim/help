...
By default, self-hosted instances of Bitwarden will use a Microsoft SQL Server (MSSQL) database created as a normal part of [installation setup](/install-on-premise), however you configure Bitwarden to use an external MSSQL database.
...
6. Run `./bitwarden.sh updatedb` to migrate the **database schema**.
   ...
   {% callout success %}`updatedb` does not migrate data, it only migrates the database schema. To move existing data to the new database, [restore a backup](/backup-on-premise/#restore-a-nightly-backup) from `./bwdata/mssql/backups`.{% endcallout %}
{% endcomment %}
