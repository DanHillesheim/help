...
Use the `login` command to login to Directory Connector with your [Organization API Key](/public-api/#authentication).  If you don't have the API Key, reach out to an [Organization Owner](/user-types-access-control/). There are a few ways to use the `login` command:
