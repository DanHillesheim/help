...
This command will initiate a prompt for your **Email address**, **Master password**, and (if [enabled](/setup-two-step-login/)) a **Two-step login code**.
...
{% callout success %}
**Getting prompted for additional authentication** or getting a `Your authentication request appears to be coming from a bot.` error? Use your API Key `client_secret` to answer the authentication challenge. [Learn more](/cli-auth-challenges/).
{% endcallout %}

Logging in with a [Personal API Key](/personal-api-key/) authenticates you with Bitwarden servers, syncs your Vault, but **does not unlock your Vault**. After logging in with an API key, you will be required to unlock your Vault using your Master Password. To log in with an API key use:
...
Logging in with [SSO](/about-sso/) authenticates you with Bitwarden servers, syncs your Vault, but **does not unlock your Vault**. After logging in with SSO, you will be required to unlock your Vault using your Master Password. To log in with SSO use:
...
The CLI currently supports [two-step login](/setup-two-step-login/) via [authenticator](/setup-two-step-login-authenticator/), [email](/setup-two-step-login-email/), or [Yubikey](/setup-two-step-login-yubikey/). If you have one of these methods enabled, you will be required to enter your two-step login code to log in. If you have multiple methods enabled, you will be prompted first to select which method to use.
...
By default, `delete` will "soft delete" an item (i.e. send it to the [Trash](/managing-items/#items-in-the-trash)). You can permanently delete an item using the `-p, --permanent` option.
...
The `send` command creates a [Bitwarden Send](/about-send) object for ephemeral sharing. This section will detail simple `send` operations, however Send is a highly flexible tool and we recommend referring to the dedicated article on [Send from CLI](/send-cli).
...
The `receive` command accesses a [Bitwarden Send](/about-send) object. To receive a Send object:
...
Accessing an Organization from the CLI frequently requires knowledge of an ID for your Organization, as well as IDs for individual [members](/managing-users/) and [Collections](/about-collections/).
...
{% callout info %}
**August 2021**: The `share` command has been changed to `move`. [Find out more](/releasenotes/).
{% endcallout %}

The `move` command transfers a Vault item [to an Organization](/sharing/):
...
The `confirm` command confirms [invited members](/managing-users/#confirm-invited-users) to your Organization who have accepted their invitation:
...
A primary use of `bw config` is to [connect your CLI to a self-hosted](/change-client-environment/#cli) Bitwarden server:
...
{% callout success %}
Bitwarden supports lots of formats for import, too many to list here! Use `bw import --formats` to return the list in your CLI, or [see here](/import-faqs/#q-what-file-formats-does-bitwarden-support-for-import).
{% endcallout %}
...
Used to specify which [Two-step Login method](/setup-two-step-login/) to use when [logging in](#log-in):
...
Used with the `create` command to specify a [Vault item type](/managing-items/):
...
Used with the `create` and `edit` commands to specify [URI match detection](/uri-match-detection/) behavior:
...
Used with the `create` and `edit` commands to configure [custom fields](/custom-fields/):
...
Indicates a [user's type](/user-types-access-control/):
...
Indicates a user's [status within the Organization](/managing-users/):
